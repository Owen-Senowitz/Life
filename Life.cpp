...
int main()
{
	...
	//creates Array to store userInput
	const int INPUTSIZE = 2;
	char userInput[INPUTSIZE];

	getFile(world); // calls function to open file with the world create above
	...
	do
	{
		...
		//statement to whether to contuine generations or to quit
...
int neighborCount(State world[][MAXCOLS], int row, int col)
{
	...
	// i is the count of neighbors surrounding world[row][col]
	int i = 0;

	//loop to count all 8 postiions that a neighbor cell can have
	for (int x = -1; x <= 1; x++)
	{
		for (int y = -1; y <= 1; y++)
		{
			if (x == 0 && y == 0)
			{
				// don't count position [row][col]
			}
			else if (world[row + x][col + y] == State::alive)
			{
				i++;
			}
		}
...
void calcNewWorld(State world[][MAXCOLS])
{
	...
	State tempArray[MAXROWS][MAXCOLS];

	for (int i = 1; i <= MAXWORLDROWS; i++) {
		for (int x = 1; x <= MAXWORLDCOLS; x++) {

			// count the number of neighbors around world[i][x]
			// and compare the counts

			int count = neighborCount(world, i, x);

			if (count == 2)
			{
				tempArray[i][x] = world[i][x];
			}
			else if (count == 3)
			{
				tempArray[i][x] = State::alive;
			}
			else // count == 0, 1, or >= 4
			{
				tempArray[i][x] = State::dead;
			}
		}
	}

	// Now need to copy all of the tempArray positions into world
	for (int i = 1; i <= MAXWORLDROWS; i++) {
		for (int x = 1; x <= MAXWORLDCOLS; x++) {

			world[i][x] = tempArray[i][x];
...
void getFile(State world[][MAXCOLS])
{
	...
	//Ask user to get file name and opens it
	...
	if (inFile.is_open()) {
		...
		while (row <= MAXWORLDROWS && inFile.getline(tempArray, MAXWORLDCOLS + 1)) {

			size_t length = strlen(tempArray);

			//reads each in a line
			for (int k = 0; k <= length; k++) {

				if (tempArray[k] == '*') {
					world[row][k + 1] = State::alive;
				}
			}
			row++;
		}
...
void initWorld(State world[][MAXCOLS])
{
	...
	{
		//initialize all columns for each row to dead state.
		for (int x = 0; x < MAXCOLS; x++)
		{
			world[i][x] = State::dead;
		}
	}
}